	...
	<tbody>
		...
		<tr style="border-collapse:collapse;">
			...
			<div style="margin-left: 40px; margin-top: 10px; ">
				...
				<a href="'.$params['url'].'?comment='.$account['token_hash'].':'.$emails.':'.$params['notifapp'].':'.$params['asset_id'].'#comment_'.$params['asset_id'].'" style="color:#a94442;">Responder</a></div>
		...
		<tr style="border-collapse:collapse;">
			<td style="border-collapse:collapse;color:#353a3d;font-size:16px;font-weight:400;word-wrap:break-word;line-height:1.4;font-family:Helvetica Neue, Helvetica, Helvetica, Arial, sans-serif;" colspan="2">
				<hr style="border:none;height:2px;overflow:hidden;background:#e8d094;">
				<div style="font-size:13px;word-break:break-all;"> No funciona el botón? Pega este enlace en tu navegador:<br> <a href="'.$params['url'].'?comment='.$account['token_hash'].':'.$emails.':'.$params['notifapp'].':'.$params['asset_id'].'#comment_'.$params['asset_id'].'" style="color:#a94442;text-decoration:none;font-size:80%;" target="_blank">'.$params['url'].'?comment='.$account['token_hash'].':'.$emails.':'.$params['notifapp'].':'.$params['asset_id'].'#comment_'.$params['asset_id'].'</a><img src="'.URLINDEX.'?email='.$account['token_hash'].':'.$emails.':comment" style="width:1px;height:1px"></div>
			</td>
		</tr>
		<tr style="border-collapse:collapse;">
